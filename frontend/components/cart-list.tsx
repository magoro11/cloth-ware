...
import { PurchaseCheckoutButton } from "@/frontend/components/purchase-checkout-button";
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1fr,320px]">
      <div className="space-y-4">
        {items.map((entry) => (
          <article key={entry.id} className="grid gap-4 rounded-2xl border border-black/5 p-3 dark:border-white/10 md:grid-cols-[120px,1fr]">
            <Link href={`/item/${entry.item.id}`} className="overflow-hidden rounded-xl bg-black/5 dark:bg-white/5">
            ...
            <div className="flex flex-col justify-between gap-3">
              <div>
                <p className="text-[11px] uppercase tracking-widest opacity-60">{entry.item.brand}</p>
                <Link href={`/item/${entry.item.id}`} className="mt-1 block font-semibold hover:opacity-70">
                  {entry.item.title}
                </Link>
                <p className="mt-2 text-sm font-semibold">{formatCurrency(entry.item.sellingPrice ?? 0)}</p>
              </div>
              <button
                onClick={() => removeItem(entry.itemId)}
                disabled={busyId === entry.itemId}
                className="w-max rounded-lg border border-black/10 px-3 py-2 text-xs hover:bg-black/5 disabled:opacity-50 dark:border-white/20 dark:hover:bg-white/10"
              >
      ...
      <aside className="h-fit rounded-2xl border border-black/5 p-5 dark:border-white/10">
        ...
        <p className="mt-4 text-xs opacity-65">Secure checkout runs through Stripe.</p>
